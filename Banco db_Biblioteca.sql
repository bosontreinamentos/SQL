...
SELECT L.NomeLivro AS Livro, L.PrecoLivro Preço, E.NomeEditora AS Editora
FROM tbl_Livro L
...
SELECT L.NomeLivro AS Livro, L.PrecoLivro,
CONCAT(A.NomeAutor, ' ', A.SobrenomeAutor) AS 'Nome do Autor',
E.NomeEditora Editora, C.NomeAssunto Assunto
FROM tbl_livro L
INNER JOIN tbl_editora E
ON L.IdEditora = E.IdEditora
INNER JOIN tbl_assunto C
ON L.IdAssunto = C.IdAssunto
INNER JOIN tbl_autor_livro AL
ON L.IdLivro = AL.IdLivro
INNER JOIN tbl_autor A
ON A.IdAutor = AL.IdAutor
WHERE L.PrecoLivro BETWEEN 55.00 AND 65.00
